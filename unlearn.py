...
from training.triplane_un import UnTriPlaneGenerator
...
def unlearn(*args, **kwargs):
    ...
    g_source = TriPlaneGenerator(*g_ema.init_args, **g_ema.init_kwargs).requires_grad_(False).to(device)
    copy_params_and_buffers(g_ema, g_source, require_all=False)
    g_source.neural_rendering_resolution = g_ema.neural_rendering_resolution
    g_source.rendering_kwargs = g_ema.rendering_kwargs
    g_source.load_state_dict(g_ema.state_dict(), strict=False)
    g_source.eval()        
    
    generator = UnTriPlaneGenerator(*g_ema.init_args, **g_ema.init_kwargs).requires_grad_(False).to(device)
    copy_params_and_buffers(g_ema, generator, require_all=False)
    generator.neural_rendering_resolution = g_ema.neural_rendering_resolution
    generator.rendering_kwargs = g_ema.rendering_kwargs
    generator.load_state_dict(g_ema.state_dict(), strict=False)
    generator.train()
    ...
    for name, param in generator.named_parameters():
        if "unlear" in name and "backbone.synthesis" in name:
    ...
    for i in pbar:
        ...
        if i % 10 == 0:
            print(f"[{i}] Total Loss: {loss.item():.4f} | "
                f"Local: {loss_dict.get('loss_local', 0):.4f}, "
                f"Adj: {loss_dict.get('loss_adj', 0):.4f}, "
                f"Global: {loss_dict.get('loss_global', 0):.4f}")
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
